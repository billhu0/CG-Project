...
void NURBSCurve::setControlPointAndWeight(int i, Vec3f control_ponit, float w) {
    control_points_[i] = std::move(control_ponit);
    w_[i] = w;
}

void NURBSCurve::setKnot(int i, float knot) {
    knots_[i] = knot;
}

void NURBSCurve::setKnots(std::vector<float> knots) {
    knots_ = std::move(knots);
}
...
[[maybe_unused]] void NURBSCurve::setKnotUniform() {
    ...
    for (int i = 0; i < knots_.size(); ++i) {
        if (i < degree_)
            ...
            knots_[i] = (float)(i - degree_) * delta;
    }
}

std::pair<Vertex, float> NURBSCurve::evaluate(float t) {
    int k = Findk(t);
    std::vector<Vec4f> P;
    Vertex res;
    float w;
    for (int i = 0; i < degree_ + 1; ++i) {
        P.emplace_back(Vec4f(control_points_[i + k - degree_].x(), control_points_[i + k - degree_].y(),
                          control_points_[i + k - degree_].z(), 1.0f) * w_[i + k - degree_]);
    }
    for (int j = 1; j <= degree_; ++j) {
        ...
        if (j == degree_ - 1)
            res.normal = Vec3f(P[degree_].x() / P[degree_].w(), P[degree_].y() / P[degree_].w(),
                               P[degree_].z() / P[degree_].w()) -
                         Vec3f(P[degree_ - 1].x() / P[degree_ - 1].w(), P[degree_ - 1].y() / P[degree_ - 1].w(),
                               P[degree_ - 1].z() / P[degree_ - 1].w());
        if (j == degree_) {
            res.position = Vec3f(P[degree_].x() / P[degree_].w(), P[degree_].y() / P[degree_].w(),
                                 P[degree_].z() / P[degree_].w());
            w = P[degree_].w();
        }
    }
...
void NURBSSurface::setKnotM(int i, float knot) {
    knots_m_[i] = knot;
}

void NURBSSurface::setKnotN(int i, float knot) {
    knots_n_[i] = knot;
}
...
std::shared_ptr<TriangleMesh> NURBSSurface::genMesh_triangle(const Vec3f &translation, float scale) {
    ...
    for (int i = 0; i <= divide_num; ++i) {
        ...
        for (int j = 0; j <= divide_num; ++j) {
            ...
            vertices.emplace_back(res.position * scale + translation);
            ...
            // std::cout << evaluate(u, v).position.x << " " << evaluate(u, v).position.y << " " << evaluate(u,
            // v).position.z << "\n";
        }
    }
...
void NURBSSurface::setKnotMUniform() {
    ...
    for (int i = 0; i < knots_m_.size(); ++i) {
        if (i < degree_m_)
            knots_m_[i] = (float)0.0;
        else if (i > n)
            knots_m_[i] = (float)1.0;
        else
            knots_m_[i] = (float)(i - degree_m_) * delta;
    }
}
...
void NURBSSurface::setKnotNUniform() {
    ...
    for (int i = 0; i < knots_n_.size(); ++i) {
        if (i < degree_n_)
            knots_n_[i] = (float)0.0;
        else if (i > n)
            knots_n_[i] = (float)1.0;
        else
            knots_n_[i] = (float)(i - degree_n_) * delta;
    }
}

std::shared_ptr<PatchMesh> NURBSSurface::genMesh_patch(const Vec3f &translation, float scale) {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        A_m[i][0] = Vec3f(0, 0, 0);
        for (int j = 1; j < n; ++j) {
            V_m[i][j] = degree_n_ * (control_points_m_[i][j] - control_points_m_[i][j - 1]) /
                        (knots_n_[j + degree_n_] - knots_n_[j]);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        A_n[i][0] = Vec3f(0, 0, 0);
        for (int j = 1; j < m; ++j) {
            V_n[i][j] = degree_m_ * (control_points_n_[i][j] - control_points_n_[i][j - 1]) /
                        (knots_m_[j + degree_m_] - knots_m_[j]);
    ...
    // printf("\n");
    for (int i = 0; i < temp_knots.size() - 1; ++i) {
        if (temp_knots[i] == left || temp_knots[i] == right)
            continue;
        else
            KnotInsertU(temp_knots[i], degree_m_);
    }
    // for(auto knot : knots_m_) printf("%f ", knot);
    // printf("\n");
    left = knots_n_[0];
    right = knots_n_[knots_n_.size() - 1];
    temp_knots = knots_n_;
    for (int i = 0; i < temp_knots.size() - 1; ++i) {
    ...
    temp_knots = knots_n_;
    for (int i = 0; i < temp_knots.size() - 1; ++i) {
        if (temp_knots[i] - left < EPS || right - temp_knots[i] < EPS)
            continue;
        else
            KnotInsertV(temp_knots[i], degree_n_);
    }
    ...
    for (int i = 0; i < knots_m_.size() - 1; ++i) {
        ...
        for (int j = 0; j < knots_n_.size() - 1; ++j) {
            ...
            NURBSPatch patch(degree_m_ + 1, degree_n_ + 1, Vec2f(knots_m_[i], knots_m_[i + 1]),
                             Vec2f(knots_n_[j], knots_n_[j + 1]));
            int a = 0, b = 0;
...
int NURBSSurface::findSpan(int degree, const std::vector<float> &knots, float t) {
...
unsigned int NURBSSurface::knotMultiplicity(const std::vector<float> &knots, float t) {
...
void NURBSSurface::KnotInsertU(float u, int reapeat) {
    ...
    for (int j = 1; j <= reapeat; ++j) {
        ...
        for (int i = 0; i <= degree_m_ - j - s; ++i)
            alpha[i][j] = (u - knots_m_[L + i]) / (knots_m_[i + span + 1] - knots_m_[L + i]);
    }

    std::vector<Vec3f> temp_cp;
    temp_cp.resize(degree_m_ + 1);
    std::vector<float> temp_w;
    temp_w.resize(degree_m_ + 1);
    ...
    for (int col = 0; col < n; ++col) {
        ...
        for (int j = 1; j <= reapeat; ++j) {
            ...
            for (int i = 0; i <= degree_m_ - j - s; ++i) {
                float a = alpha[i][j];
                Vec4f cpi(temp_cp[i].x() * temp_w[i], temp_cp[i].y() * temp_w[i], temp_cp[i].z() * temp_w[i],
                          temp_w[i]);
                Vec4f cpi1(temp_cp[i + 1].x() * temp_w[i + 1], temp_cp[i + 1].y() * temp_w[i + 1],
                           temp_cp[i + 1].z() * temp_w[i + 1], temp_w[i + 1]);
                Vec4f ncp = (1 - a) * cpi + a * cpi1;
                temp_cp[i] = Vec3f(ncp.x(), ncp.y(), ncp.z()) / ncp.w();
                temp_w[i] = ncp.w();
            }
            new_cp[L][col] = temp_cp[0];
            new_w[L][col] = temp_w[0];
...
void NURBSSurface::KnotInsertV(float v, int reapeat) {
    ...
    for (int j = 1; j <= reapeat; ++j) {
        ...
        for (int i = 0; i <= degree_n_ - j - s; ++i)
            alpha[i][j] = (v - knots_n_[L + i]) / (knots_n_[i + span + 1] - knots_n_[L + i]);
    }

    std::vector<Vec3f> temp_cp;
    temp_cp.resize(degree_n_ + 1);
    std::vector<float> temp_w;
    temp_w.resize(degree_n_ + 1);
    ...
    for (int row = 0; row < m; ++row) {
        ...
        for (int j = 1; j <= reapeat; ++j) {
            ...
            new_cp[row][L] = temp_cp[0];
            new_w[row][L] = temp_w[0];
